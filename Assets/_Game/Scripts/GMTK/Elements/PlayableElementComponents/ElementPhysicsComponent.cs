  ...
  public class ElementPhysicsComponent : PlayableElementComponent {
    ...
    private void ApplyExtensionRotation(bool clockwise, bool skipPermissionCheck = false) {
      ...
      if (!skipPermissionCheck || (AllowsRotationChanges && _rigidbody2D.bodyType != RigidbodyType2D.Static)) {
        ...
        this.LogDebug($"Applied extension rotation {(clockwise ? "CW" : "CCW")} to {newRotation}� with step {RotationStep}� (flippedX: {_playableElement.ModelTransform.IsFlippedX()}, flippedY: {_playableElement.ModelTransform.IsFlippedY()})");
