...
namespace GMTK {
  ...
  public class LevelGrid : MonoBehaviour {
    ...
    [SerializeField] private bool drawOccupancyGizmos = true;
    ...
    public void OnValidate() {
      ...
      GridSize.x = Mathf.Clamp(GridSize.x, 4, 100);
      GridSize.y = Mathf.Clamp(GridSize.y, 4, 100);
    }
    ...
    protected virtual void InitializeAllEdgeColliderBounds() {
      ...
      //the EdgeColliders are snapped to the edges of the grid, base on the tag
    ...
    private EdgeCollider2D UpdateEdgeColliderBoundPoints(EdgeCollider2D boundCollider, string tag) {
      ...
      //Reset the collider points to ensure they are surrounding the grid edges
    ...
    protected virtual void InitializeGrid() {
      ...
      foreach (var snappable in allSnappables) {
        if (IsInsidePlayableArea(snappable.transform.position)) {
          ...
          snappable.SetStatic();
          _occupancyMap.Register(snappable);
        }
      }
    ...
    private void HandleElementUnhovered(object sender, GridSnappableEventArgs e) {
      if (e.Element != null) e.Element.SetHovered(false);
    }

    protected virtual void HandleElementHovered(object sender, GridSnappableEventArgs e) {
      if (e.Element != null) e.Element.SetHovered(true);
    }
    #endregion
    ...
    public bool IsOccupied(Vector2 position) => _occupancyMap.IsOccupied(position);

    protected virtual bool CanPlace(GridSnappable snappable, Vector2Int gridOrigin) {
      foreach (var cell in snappable.GetWorldOccupiedCells(gridOrigin)) {
        if (_occupancyMap.IsOccupied(cell)) return false;
      }
      return true;
    }
    ...
    /// <summary>
    /// Calculates the Grid coordinates that correspond to the world position 'position' and returns them as a new Vector2 
    /// </summary>
    ...
    /// <summary>
    /// Calculates the Grid position that corresponds to the world position specified in 'position'
    /// </summary>
    /// <param name="position"></param>
    ...
    private void OnDrawGizmosSelected() {
      if (!drawOccupancyGizmos || _occupancyMap == null) return;
      ...
      foreach (var kvp in _occupancyMap.GetAllCells()) {
        ...
        Gizmos.color = occupants.IsFull ? occupiedColor : freeColor;
