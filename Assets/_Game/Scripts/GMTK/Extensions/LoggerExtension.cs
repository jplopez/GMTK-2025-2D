...
namespace GMTK {
  ...
  public static class LoggerExtension {
    ...
    public static string GetTimestamp() => DateTime.Now.ToString(DateFormat);

    public static bool CanLog(LoggerLevels level) => !DisableAllLogging && EnabledLogLevels.TryGetValue(level, out var isEnabled) && isEnabled;
    
    public static void SetLogLevel(LoggerLevels level, bool isEnabled) => EnabledLogLevels[level] = isEnabled;
    ...
    private static readonly Dictionary<string, string> _stackedLogs = new();

    public static void StackLogMessage(string key, string message)
    {
      _stackedLogs.TryAdd(key, "");
