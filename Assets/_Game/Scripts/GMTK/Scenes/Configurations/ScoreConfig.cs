...
namespace GMTK {
  ...
  public class ScoreConfig : MonoBehaviour, ISceneConfigExtension {
    ...
    [Tooltip("the reference to the ScoreGateKeeper to configure. If empty, this Config will look for one through 'ServiceLocator'")]
    ...
    public void ApplyConfig(SceneController controller) {
      ...
      if(scoreKeeper == null) {
        scoreKeeper = ServiceLocator.Get<ScoreGateKeeper>();
      }
      if(scoreKeeper == null) {
        Debug.LogError($"{name} : can't find an instance of ScoreGateKeeper. Make sure the ServiceLocator system has definition for this type");
        return;
      }
